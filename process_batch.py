...
#     parser.add_argument("--on_jet_tag_min_pt", type=float, required=False)
    ...
    # separate into batch
    batch_size = configs["IO"]["batch_size"]
    assert batch_size > 0 and (isinstance(batch_size, int) or batch_size < 1)
    fileset = json2dict(fileset_json_path)
    if batch_size < 1:
        splits = np.array_split()
    else:
        splits = np.array_split()
    for split_idx, split in enumerate(splits):
        dict2json({dataset_name + "_{}".format(split_idx): split})
